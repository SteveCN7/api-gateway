...
class DelegatedAuthorityFilterSpec extends UnitSpec with MockitoSugar {
  ...
  private val proxyRequest = ProxyRequest("GET", "/hello/world", headers = Map(AUTHORIZATION -> "Bearer 31c99f9482de49544c6cc3374c378028"))
  ...
  "Delegated authority filter" should {
    ...
    "throw an exception when credentials are missing" in {
      val requestWithoutAuthorizationHeader = proxyRequest.copy(headers = Map())

      intercept[MissingCredentials] {
        await(authorityService.findAuthority(requestWithoutAuthorizationHeader))
      }
    }
